...
export default function GenesisSealBadge() {
  ...
  useEffect(() => {
    const fetchGenesisSeal = async () => {
      try {
        const proof = await base44.entities.ZKProof.list('-created_date', 1);
        if (proof && proof.length > 0) {
          const genesisProof = proof.find(p => p.id === '69c3079bcfacdd458de60765') || proof[0];
          setZkProof(genesisProof);
        }
      } catch (e) {
        console.log('Genesis Seal fetch complete');
      } finally {
        setLoading(false);
      }
    };
