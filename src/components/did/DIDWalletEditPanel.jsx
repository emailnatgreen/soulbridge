...
export default function DIDWalletEditPanel({ wallet, agents, onSave, onClose }) {
  ...
  const handleSave = async () => {
    setSaving(true);
    const notesData = { ...parsed, role, color, linkedAgent: linkedAgent || null, linkedNodeDid: linkedNodeDid || null };
  ...
  return (
    <div className="bg-white/5 border border-purple-500/30 rounded-xl p-4 space-y-3 animate-in fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          <label className="text-white/40 text-[10px] mb-1 block">Name</label>
        ...
        <div>
          <label className="text-white/40 text-[10px] mb-1 block flex items-center gap-1"><Shield className="w-3 h-3" /> Role</label>
          <select value={role} onChange={e => setRole(e.target.value)}
            ...
            {roles.map(r => <option key={r} value={r} className="bg-slate-900">{r.charAt(0).toUpperCase() + r.slice(1)}</option>)}
          </select>
        </div>
        <div>
          <label className="text-white/40 text-[10px] mb-1 block flex items-center gap-1"><Users className="w-3 h-3" /> Link Agent</label>
          <select value={linkedAgent} onChange={e => setLinkedAgent(e.target.value)}
            className="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-purple-400/60 appearance-none">
        ...
        <div>
          <label className="text-white/40 text-[10px] mb-1 block flex items-center gap-1"><Globe className="w-3 h-3" /> Node DID Link</label>
          <input type="text" value={linkedNodeDid} onChange={e => setLinkedNodeDid(e.target.value)}
      ...
      <div className="flex items-center gap-2">
        ...
        <Button size="sm" variant="ghost" onClick={onClose} className="text-white/40 hover:text-white text-xs">Cancel</Button>
      </div>
    </div>
  );
}