...
// Flow classification that matches real economic meaning
...
// ACQUISITION = agent acquires resource (cost): resource_acquired
// NEUTRAL = swap/trade: traded
const FLOW_CONFIG = {
...
// Filter out simulated mega-transactions (same logic as main dashboard)
function isRealisticAmount(a) {
  // Internal task reward hashes with unrealistic amounts
  if (a.transaction_hash?.startsWith('TASK_') && a.amount > 1000) return false;
  // Anything over 10k XRP without a real tx hash is suspect
...
export default function EarnersTab({ activities = [], agents = [] }) {
  // Only use completed + realistic activities
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Top Earners Chart */}
      {inflows.length > 0 && (
        <ChartCard title="Top Agent Earnings" data={aggregateByAgent(inflows, agents)} color="#22c55e" label="Earned (XRP)" />
      )}

      {/* Treasury Deposits Chart */}
      {deposits.length > 0 && (
        <ChartCard title="Top Treasury Depositors" data={aggregateByAgent(deposits, agents)} color="#3b82f6" label="Deposited (XRP)" />
      )}

      {/* Outflows Chart */}
      {outflows.length > 0 && (
        <ChartCard title="Top Agent Outflows" data={aggregateByAgent(outflows, agents)} color="#ef4444" label="Outflow (XRP)" />
      )}

      {/* Swaps Chart */}
      {swaps.length > 0 && (
        <ChartCard title="Trade / Swap Activity" data={aggregateByAgent(swaps, agents)} color="#6366f1" label="Traded (XRP)" />
      )}
...
function NetBalanceTable({ activities, agents }) {
  // Calculate net balance: earnings - spending per agent
  const balanceMap = {};
