...
const TYPE_VISUALS = {
  ...
  spent: { icon: ArrowDownRight, iconColor: 'text-red-400', iconBg: 'bg-red-500/15 border-red-500/30', amountColor: 'text-red-400', prefix: '−', tagColor: 'bg-red-500/15 text-red-400 border-red-500/30' },
  treasury_withdrawal: { icon: Landmark, iconColor: 'text-amber-400', iconBg: 'bg-amber-500/15 border-amber-500/30', amountColor: 'text-amber-400', prefix: '−', tagColor: 'bg-amber-500/15 text-amber-400 border-amber-500/30' },
  treasury_deposit: { icon: Landmark, iconColor: 'text-blue-400', iconBg: 'bg-blue-500/15 border-blue-500/30', amountColor: 'text-blue-400', prefix: '→', tagColor: 'bg-blue-500/15 text-blue-400 border-blue-500/30' },
  resource_acquired: { icon: Package, iconColor: 'text-cyan-400', iconBg: 'bg-cyan-500/15 border-cyan-500/30', amountColor: 'text-cyan-400', prefix: '−', tagColor: 'bg-cyan-500/15 text-cyan-400 border-cyan-500/30' },
  traded: { icon: RefreshCw, iconColor: 'text-indigo-400', iconBg: 'bg-indigo-500/15 border-indigo-500/30', amountColor: 'text-indigo-400', prefix: '⇄', tagColor: 'bg-indigo-500/15 text-indigo-400 border-indigo-500/30' },
};

const DEFAULT_VISUAL = { icon: Wallet, iconColor: 'text-slate-400', iconBg: 'bg-slate-500/15 border-slate-500/30', amountColor: 'text-slate-400', prefix: '', tagColor: 'bg-slate-500/15 text-slate-400 border-slate-500/30' };
  ...
  return (
    <div className="space-y-4">
          ...
          {types.map(t => {
            ...
            return (
              <button key={t} onClick={() => setFilter(t)}
                className={`text-xs px-3 py-1.5 rounded-lg transition-all flex items-center gap-1.5 ${isActive ? 'bg-emerald-600 text-white shadow-md shadow-emerald-900/30' : 'bg-slate-800/80 text-slate-400 hover:text-white border border-slate-700/60 hover:border-slate-600'}`}>
                {vis && <vis.icon className="w-3 h-3" />}
                {label}
                <span className={`text-[10px] px-1 rounded ${isActive ? 'bg-white/20' : 'bg-slate-700/60'}`}>{count}</span>
              </button>
            );
          })}
        </div>
      ...
      <p className="text-slate-500 text-xs">
        ...
        {filter !== 'all' && (
          <button onClick={() => setFilter('all')} className="ml-2 text-emerald-400 hover:text-emerald-300 underline">Clear filter</button>
        )}
      </p>
    ...
    <div className="flex items-center gap-3 p-3 bg-slate-900/60 border border-slate-700/40 rounded-xl hover:border-slate-600/60 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          <AgentLink agentId={activity.agent_id} agents={agents} className="text-sm font-medium" />
          {activity.related_agent_id && (
            <span className="text-slate-500 text-xs">→ <AgentLink agentId={activity.related_agent_id} agents={agents} className="text-xs" /></span>
          )}
        ...
        {activity.transaction_hash && activity.transaction_hash !== 'N/A - manual confirmation' && (
          <p className="text-slate-600 text-[10px] font-mono mt-0.5 truncate">tx: {activity.transaction_hash.slice(0, 16)}…</p>
        )}
      </div>
